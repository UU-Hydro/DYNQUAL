...
class Routing(object):
    ...
    def getState(self):
        ...
        try:
            ...
            try:
                ...
                logger.info("Routing pollutant loadings from each sector individually, as well as combined")
                #Domestic sector
    ...
    def __init__(self,iniItems,initialConditions,lddMap):
        ...
        if self.quality:
            ...
            if self.calculateLoads:
                ...
                self.Buffalo_BODload = vos.netcdf2PCRobjCloneWithoutTime(self.Liv_ExcrLoadNC,"Buffalo_BODload",self.cloneMap) # g/stock/day
                ...
                self.Buffalo_FCload = vos.netcdf2PCRobjCloneWithoutTime(self.Liv_ExcrLoadNC,"Buffalo_FCload",self.cloneMap) # cfu/stock/day
    ...
    def readExtensiveHydro(self, currTimeStep):
        ...
        #Options for reading in annual hydrology files (daily timestep)
        if self.hydrology_set_per_year:
            baseflowNC = self.baseflowNC %(int(currTimeStep.year), int(currTimeStep.year))
            interflowNC = self.interflowNC %(int(currTimeStep.year), int(currTimeStep.year))
            directRunoffNC = self.directRunoffNC %(int(currTimeStep.year), int(currTimeStep.year))
        else:
            baseflowNC = self.baseflowNC
            interflowNC = self.interflowNC
            directRunoffNC = self.directRunoffNC    
    ...
    def readPollutantLoadingsInputData(self, currTimeStep):
        ...
        self.BuffaloPopulation = vos.netcdf2PCRobjClone(\
                                 self.LivPopulationNC,'BuffaloPop',\
                                 str(currTimeStep.fulldate), 
                                 useDoy = None,
                                  cloneMapFileName=self.cloneMap,\
                                  LatitudeLongitude = True,specificFillValue = None) #input pathway for buffalo population (5 arc-mins)
