...
class LandCover(object):
    ...
    def __init__(self,iniItems,nameOfSectionInIniFile,parameters,landmask,usingAllocSegments = False):
        ...
        self.parameters = parameters #TODO - adjusting land surface
    ...
    def updateLC(self,meteo,groundwater,routing,\
                 ...
                 nonIrrGrossDemandDict,swAbstractionFraction,\
                 currTimeStep,\
                 allocSegments,\
    ...
    def calculateWaterDemand(self, parameters, nonIrrGrossDemandDict, swAbstractionFraction, groundwater, routing, allocSegments, currTimeStep,desalinationWaterUse):
        ...
        # non irrigation water demand #TODO ED
            ...
            # non-irrigation water demand (domestic, industry and livestock)
            ...
            self.DomGrossDemand = nonIrrGrossDemandDict['potential_demand']['domestic']
            self.IndGrossDemand = nonIrrGrossDemandDict['potential_demand']['industry']
            self.LivGrossDemand = nonIrrGrossDemandDict['potential_demand']['livestock]
            self.nonIrrGrossDemand = self.DomGrossDemand +\
                                     self.IndGrossDemand +\
                                     self.LivGrossDemand
        ...
        self.totalIrrigationLivestockDemand = self.irrGrossDemand + self.LivGrossDemand
        ...
        #Water demands satisified by desalination
        satisfiedDomesticDemand     = vos.getValDivZero(self.DomGrossDemand, self.totalPotentialGrossDemand) * self.desalinationAllocation
        satisfiedIndustryDemand     = pcr.max(0.00, self.desalinationAllocation - satisfiedDomesticDemand)
        satisfiedLivestockDemand     = pcr.max(0.00, self.desalinationAllocation - (satisfiedDomesticDemand + satisfiedIndustryDemand))
        satisfiedIrrigationDemand = pcr.max(0.00, self.desalinationAllocation - (satisfiedDomesticDemand + satisfiedIndustryDemand + satisfiedLivestockDemand))

        self.totalGrossDemandAfterDesalination = pcr.max(0.0, self.totalPotentialGrossDemand - self.desalinationAllocation)
        surfaceGrossDemandAfterDesalination = self.totalGrossDemandAfterDesalination * swAbstractionFraction
        
        # remaining water demand per sector
        remainingDomestic   = pcr.max(0.0, self.DomGrossDemand  - satisfiedDomesticDemand)
        remainingIndustry   = pcr.max(0.0, self.IndGrossDemand  - satisfiedIndustryDemand)
        remainingLivestock  = pcr.max(0.0, self.LivGrossDemand - satisfiedLivestockDemand)
        ...
        if self.usingAllocSegments:      # using zone/segment at which supply network is defined
            ...
            cellVolGrossDemand = pcr.rounddown(surfaceGrossDemandAfterDesalination*routing.cellArea)
            ...
            self.actSurfaceWaterAbstract = pcr.min(routing.readAvlChannelStorage/routing.cellArea,\
                                                   surfaceGrossDemandAfterDesalination)  # unit: m
